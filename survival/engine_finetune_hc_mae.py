...
def train_one_epoch(model: torch.nn.Module,
    ...
    survival_accum = None
    k = -1
    for data_iter_step, (imgs, vecs_reg, seq_len, X_mrna, X_mirna, X_meth, censored, survival) \
        ...
        with torch.cuda.amp.autocast():
            samples = [imgs, vecs_reg, seq_len, X_mrna, X_mirna, X_meth]
...
def evaluate(data_loader, model, device):
    ...
    output_all, censored_all, survival_all = torch.tensor([], device=device), \
                                             ...
                                             torch.tensor([], device=device)
    for data_iter_step, (imgs, vecs_reg, seq_len, X_mrna, X_mirna, X_meth, censored, survival) in enumerate(metric_logger.log_every(data_loader, 10, header)):
